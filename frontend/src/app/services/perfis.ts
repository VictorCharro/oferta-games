...
export interface PerfilBloco { id: string; tipo: 'resumo_favoritos' | 'horas' | 'conquistas' | 'favoritos' | 'biblioteca' | 'atividade' | 'texto' | 'imagem' | 'links'; titulo: string | null; conteudo: string | null; posicao: number; tamanho: 'pequeno' | 'medio' | 'largo' | 'completo'; visivel: boolean; tipoFundo: 'padrao' | 'cor' | 'imagem' | 'gradiente'; valorFundo: string | null; opacidade: number; corTexto: string | null; }
export interface PerfilPublico { handle: string; nomeExibicao: string; bio: string | null; avatarUrl: string | null; avatarZoom: number; avatarPosicaoX: number; avatarPosicaoY: number; totalMinutos: number | null; conquistasDesbloqueadas: number | null; conquistasTotal: number | null; totalJogosBiblioteca: number | null; biblioteca: Array<{ appId: number; titulo: string; minutosJogadas: number; iconeHash: string | null; conquistasDesbloqueadas: number; conquistasTotal: number }>; favoritos: Array<{ slug: string | null; steamAppId: number | null; titulo: string; capaUrl: string | null; iconeHash: string | null; ehDlc: boolean | null; precoMinimo: number | null; precoRegular: number | null; favoritadoEm: string }>; atividades: Array<{ tipo: string; tituloJogo: string | null; detalhe: string | null; criadaEm: string }>; mostrarAtividades: boolean; blocos: PerfilBloco[]; }
