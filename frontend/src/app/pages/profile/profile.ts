...
export class Profile implements OnInit, OnDestroy {
  ...
  private async redirecionarParaPerfilCanonico(): Promise<boolean> {
    try {
      ...
      if (!profile?.handle) {
        ...
        await this.perfisService.salvar({ handle, nomeExibicao: this.auth.displayName, bio: this.bio, publico: true, mostrarHoras: true, mostrarConquistas: true, mostrarBiblioteca: true, mostrarFavoritos: true, mostrarAtividades: true, mostrarColecoes: true });
  ...
  get activityItems(): Array<{ icon: string; title: string; time: string; tone: string }> {
    const items = this.latestMonitoredGames.map((game, index) => ({
      icon: 'jogos-monitorados-modo-escuro.png',
