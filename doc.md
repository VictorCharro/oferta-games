...
- Esse endpoint e publico (sem login) — cooldown de 5min por jogo (10/08/2026): `games.last_manual_refresh_at`, checado no inicio de `atualizarPrecos` antes de qualquer chamada externa. Dentro do cooldown, responde 429 com `{ error, segundosRestantes }` e header `Retry-After`; o frontend mostra "Aguarde Xs/min para atualizar de novo" em vez de chamar de novo. O cooldown e so no jogo principal do refresh, nao em cada DLC individualmente.
