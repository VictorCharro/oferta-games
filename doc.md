...
- **A altura e a MESMA em todos os tamanhos, nao so na mesma linha** (12/09/2026): `app-public-profile { --altura-bloco: 520px }` e `.profile-block { height: var(--altura-bloco) }`. Trocar o tamanho de um bloco muda **so a largura** (quantas das 12 colunas ele ocupa). Antes cada tamanho tinha seu proprio `min-height` (138/220/270/310px), entao Pequeno → Completo esticava o bloco e quebrava o alinhamento da linha. Dois detalhes que custaram tempo:
  ...
  - **As listas crescem PRO LADO**, nunca pra baixo: `.achievement-list` virou grid e ganha 2 colunas em largo/completo; `.profile-favorite-list` fica em 1 coluna no pequeno/medio (374px nao cabem duas capas + metas) e 2 nos maiores. As contagens foram ajustadas pro que realmente cabe: `achievementPreviewLimit` = 6/5/10/10 e a lista compacta de favoritos = 5/5/10/10.
  ...
  - **O valor do teto e a altura que a linha ja tinha, nao um numero menor**: a primeira versao usou 460px e o usuario viu o perfil encolher ("pq diminuiu a altura? esse tamanho tava bom"). A linha do Resumo media 514px antes de qualquer teto — o teto existe pra impedir o bloco de CRESCER (Biblioteca em Completo chegou a 922px), nao pra apertar o que ja estava bom. Corrigido pra 520px.
