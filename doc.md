...
- **Visibilidade independente do toggle geral de colecoes (11/09/2026)**: `profiles.show_steam_wishlist` (default `true`) controla so a wishlist, separado de `show_collections` — dono ve a propria wishlist sempre (mesmo desmarcada, pra poder reativar), so o visitante deixa de ver. Checkbox "Mostrar lista de desejos Steam" fica dentro do modo **Organizar** da aba Colecoes (nao em Configuracoes > Privacidade, diferente dos outros `mostrar*`), visivel so quando existe uma colecao de wishlist pra mostrar. `PUT /api/perfis/me/wishlist-steam` (`{ mostrar: boolean }`) e um endpoint dedicado, no mesmo padrao de `/me/avatar`/`/me/banner` — nao passa pelo `salvar()` geral do perfil.
