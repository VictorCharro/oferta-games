...
- **`vercel.json` reduzido a `{ "framework": "angular" }`** (10/08/2026, ajustado no mesmo dia apos o primeiro deploy real): o arquivo antigo forcava `outputDirectory: dist/frontend/browser` + rewrite `/(.*) -> /index.html`, tratando o site como puramente estatico. Primeira tentativa foi remover o arquivo inteiro pra deixar a deteccao automatica da Vercel assumir — **isso nao funcionou**: o projeto na Vercel tinha o Framework Preset em branco (`framework: null` via API, confirmado tanto pelo dashboard quanto pela API deles), entao sem hint nenhum o build (`vercel build`) tratou a saida como estatica generica e todas as rotas (ate a Home) devolviam 404 da propria Vercel, sem nenhum erro de runtime (a function SSR nunca chegou a ser criada). Corrigido setando `framework: "angular"` explicitamente no `vercel.json` (slug valido documentado pela Vercel) — isso faz o build reconhecer a saida `dist/frontend/browser` + `dist/frontend/server` e empacotar o servidor como function, sem precisar mexer no Framework Preset do dashboard (que o usuario nao conseguiu achar na tela de Settings).
