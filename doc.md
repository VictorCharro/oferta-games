...
- **Google Search Console (22/09/2026)**: propriedade de **prefixo de URL** em `https://ofertagames.vercel.app` (a de Dominio exige DNS, que so existe depois do dominio proprio). Verificada pelos **dois** metodos de proposito, pra derrubar um nao desverificar a propriedade: a meta `google-site-verification` no topo do `<head>` do `index.html` e o arquivo `frontend/public/googlee0a14c06e329d458.html`. **Nenhum dos dois e segredo** (toda pagina verificada serve a meta publicamente) e **nenhum pode ser removido** depois de verificado. Ambos valem so pra este dominio: na migracao (#19) a propriedade nova gera outra meta e outro arquivo. O arquivo funciona porque a Vercel resolve o filesystem antes dos rewrites do SSR — a mesma regra que obrigou a apagar o `index.csr.html` no build.
  - **Como o site aparece no resultado do Google (23/09/2026)**: a Home foi indexada em ~1 dia, mas aparecia com o nome "Vercel", icone de globo generico, titulo "Oferta Games: Início" (o Google reescreveu o nosso "Início | Oferta Games") e o texto solto da pagina no lugar da descricao. Tres causas, tres ajustes: (1) **nome do site**: JSON-LD `WebSite` na Home (`SeoService.dadosDoSite`), que e o que o Google usa no lugar do dominio; (2) **icone**: o Google so aceita favicon de lado **multiplo de 48 px** e so existiam 32 e 512, entao adicionei `favicon-48.png`, `favicon-96.png` e `/favicon.ico` (que o crawler de favicon tambem tenta por padrao; era 404); (3) **titulo e descricao**: Home com `tituloCompleto` (nome do site NA FRENTE, sem o sufixo) e descricao concreta. **Tudo isso e dica, nao ordem**: o Google decide nome, icone e snippet, e recoleta com atraso (icone costuma levar dias a semanas). Com dominio proprio o nome do site tende a sair certo sem depender disso.
  - Por que o site nao aparecia: ate 22/09/2026 ele nunca tinha sido submetido e nao tem nenhum link externo apontando pra ele, entao o Google simplesmente nunca passou. `site:ofertagames.vercel.app` devolvia zero. Nao era `noindex` nem penalidade — o SSR, o canonical e o sitemap ja estavam corretos. **Buscar no Google nao indexa nada**; quem faz isso e o Search Console mais links de fora.
...
- **`sitemap.xml`** fica no dominio do **backend**, nao no frontend (pacote novo `sitemap/`, `ControladorSitemap` + `ServicoSitemap`). Motivo: sitemap referenciado via `Sitemap:` no `robots.txt` aceita dominio cruzado (Google suporta), e gerar no backend evita duplicar a query de jogos/filtros no frontend. Como o catalogo passa de 100k jogos (acima do limite de 50.000 URLs por arquivo do protocolo de sitemap), e um indice: `GET /sitemap.xml` lista `sitemap-estatico.xml` (home/catalogo/mais-vendidos/gratuitos) + uma pagina `sitemap-jogos-N.xml` por bloco de 10.000 jogos (`ServicoSitemap.TAMANHO_PAGINA`). `robots.txt` (`frontend/public/robots.txt`, estatico) desautoriza `/login`, `/perfil`, `/configuracoes`, `/monitorados`, `/admin/` e aponta `Sitemap: https://api.163.176.220.243.sslip.io/sitemap.xml` — precisa ser atualizado se o dominio do backend mudar (hoje e o IP temporario da Oracle via sslip.io). **Dois dominios convivem aqui, e trocar um pelo outro quebra o sitemap inteiro:** os `<loc>` do *indice* apontam pros proprios sub-sitemaps, servidos pelo **backend** (`app.public-backend-url`, a mesma variavel do retorno OpenID da Steam); ja as URLs *dentro* de cada sub-sitemap sao paginas do site e usam o **frontend** (`app.frontend-url`). O indice usava a URL do frontend nos dois casos ate 01/09/2026 — o crawler pegava o indice no backend, era mandado pro frontend e recebia **HTML em vez de XML** (o frontend responde 200 com a pagina do app pra qualquer caminho desconhecido), entao nenhuma pagina do catalogo era descoberta. Coberto por teste de regressao em `ServicoSitemapTest`.
  - **O sitemap nao e o catalogo inteiro (22/09/2026)**: ate aqui listava os ~190 mil jogos ordenados por `id`, ou seja, na ordem de importacao — a `sitemap-jogos-1.xml` comecava com add-on de simulador, bundle e "super detonado", e era justamente ali que o Google gastava o orcamento de rastreamento (que e curto pra site novo). Agora `RepositorioJogos.listarJogosParaSitemap` exige **oferta em loja visivel**, exclui **DLC** e ordena por **rank** (sem rank vai pro fim, nao pra fora). Pagina que sai do sitemap continua acessivel e indexavel; ela so nao ocupa vaga. Como o rank passou a expirar (ver "Ranking de popularidade"), a pagina 1 e sempre o que esta popular hoje.
  - **`<lastmod>` vem do historico de preco, nao de `last_price_sync_at` (22/09/2026)**: a sincronizacao carimba todo jogo a cada volta da fila, mesmo sem nada mudar, entao usa-la faria **toda** URL alegar "mudei hoje" e o Google passaria a ignorar o campo no sitemap inteiro (lastmod so vale enquanto for confiavel). `price_history` so ganha linha quando o menor preco muda de verdade, que e exatamente "quando esta pagina mudou"; sem historico, cai em `games.created_at`; sem nenhuma data, **a tag nao sai** — nunca uma data inventada. Saiu o `changefreq` das paginas de jogo (o Google declarou publicamente que ignora, e so competia com o lastmod); nas quatro paginas fixas ele continua, por nao existir data unica que as represente.
  - `contarJogosParaSitemap` e cacheada: e uma contagem do catalogo inteiro com `EXISTS` por linha, numa URL publica sem login, e o controller a chama pra validar o numero da pagina — sem cache, um crawler percorrendo o indice dispararia uma por sub-sitemap.
  - O SQL desses dois metodos e coberto em `CatalogoPostgresTest` (ordem por rank, exclusao de DLC/sem oferta/loja bloqueada, origem do lastmod). **Esse arquivo so roda com `TEST_CATALOGO_JDBC` apontando pra um Postgres local e a CI nao provisiona um**, entao hoje esses testes nao rodam em lugar nenhum automaticamente — rodar na mao ao mexer nas queries.
  - **Servido tambem pelo dominio do frontend, via rewrite da Vercel (22/09/2026)**: `frontend/vercel.json` reescreve `/sitemap.xml`, `/sitemap-estatico.xml` e `/sitemap-jogos-*.xml` pro backend (proxy, nao redirect — a URL na barra do navegador/crawler continua sendo a do frontend). Motivo: o Google Search Console so aceita submeter sitemap dentro do prefixo da propriedade verificada (`https://ofertagames.vercel.app/...`), e a propriedade e de prefixo de URL, nao de dominio — verificar tambem `api.163.176.220.243.sslip.io` exigiria uma segunda propriedade so pra isso. As tres regras do sitemap **precisam vir antes** do catch-all `/(.*) -> /api` no `rewrites`, senao o SSR responde primeiro. `robots.txt` foi atualizado pra apontar `Sitemap: https://ofertagames.vercel.app/sitemap.xml`; o backend continua respondendo nos dois dominios (o `<loc>` interno do indice de sitemap continua sendo o do backend — ver acima — e funciona igual, so o **ponto de entrada** que o Search Console usa mudou).
...
**Incidente: `rank_updated_at` faltando em producao (22/09/2026).** O commit que introduziu a expiracao (20/09/2026) ja veio com `backend-java/sql/catalogo/001_validade_ranking.sql` (`ALTER TABLE games ADD COLUMN IF NOT EXISTS rank_updated_at ...`), mas a migration **nunca foi aplicada na VM** antes do deploy do codigo que passou a usa-la — job falhou com `BadSqlGrammarException` a cada rodada por dois dias, sem perda de dado (a transacao inteira da rollback, entao o rank so ficou parado no ultimo valor bom). Aplicada manualmente via SSH (`docker exec oferta-games-catalogo-db psql -U catalogo -d catalogo -c "ALTER TABLE ..."`, ver `backend-java/sql/catalogo/README.md`).

**Causa raiz, nao so o sintoma:** o banco do catalogo, diferente do Supabase, nao tem aplicacao automatica de migration (nao passa pelo MCP nem por nenhum passo do CI/CD) — o pipeline builda e reinicia o container do backend, mas nunca roda SQL contra `CATALOG_DATABASE_URL`. Uma migration em `backend-java/sql/catalogo/` so vira efeito se alguem lembrar de rodar na mao, na ordem certa, antes do deploy que depende dela — exatamente a mesma classe de risco que ja existe pras migrations do Supabase (ver "Migrations Supabase"), so que sem o lembrete automatico. **Checklist antes de publicar** ganhou um item por causa disso.
...
4. Conferir se uma migration nova precisa ser aplicada antes do deploy — **nos dois bancos**: Supabase (`backend-java/sql/`, aplicada via MCP) e o catalogo na VM (`backend-java/sql/catalogo/`, **sem aplicacao automatica nenhuma** — precisa SSH manual, ver `backend-java/sql/catalogo/README.md`; foi exatamente esquecer essa segunda parte que derrubou o job de ranking por dois dias em 22/09/2026, ver "Ranking de popularidade").
