...
O workflow `.github/workflows/deploy-oracle.yml` atualiza o backend na VM por SSH em cada push relevante para `master`; exige os segredos `ORACLE_HOST` e `ORACLE_SSH_PRIVATE_KEY` no GitHub. O bloqueio de billing do GitHub Actions (18/07 a ~31/07/2026) ja foi resolvido — o job agora inicia normalmente — mas desde entao a etapa de SSH falha com um erro diferente: `Load key "/home/runner/.ssh/id_oracle": error in libcrypto` / `Permission denied (publickey)` (verificado em 01/08/2026, run 30707545180). O secret `ORACLE_SSH_PRIVATE_KEY` provavelmente esta corrompido, no formato errado, ou expirou; precisa ser regravado no GitHub antes do deploy automatico voltar a funcionar. Ate la, o deploy manual continua sendo o caminho:
...
- **Conquistas de catalogo** (`coletarConquistasCatalogo`, ~3h — schema e % global mudam devagar): `ISteamUserStats/GetSchemaForGame/v2` (precisa de `STEAM_WEB_API_KEY`, chamado com `l=brazilian` desde 19/07/2026 — antes vinha em ingles por faltar esse parametro) mesclado com `ISteamUserStats/GetGlobalAchievementPercentagesForApp/v2` (publico). Grava em `game_achievements` (upsert por `api_name`). Nao tem relacao com `conexoes/AgendadorConquistasSteam`, que sincroniza as conquistas *desbloqueadas por um usuario logado* — este job novo e catalogo-wide, sem usuario.
  - Historico de backlog: em 19/07/2026 `game_achievements` foi zerada em producao pra reprocessar tudo em portugues (fix do `l=brazilian`) e o job foi acelerado ate zerar (concluido, revertido de volta pro ritmo normal). Em 31/07/2026 o fix do regex de `/agecheck/app/` em `ServicoSteam` (ver "Metadados Steam de catalogo" abaixo) liberou `steam_app_id` de centenas de jogos que antes ficavam bloqueados pra sempre, dobrando o backlog pendente de ~769 para ~1724; o job foi acelerado de novo (`LIMITE_CONQUISTAS_CATALOGO=250` a cada 2min, normalmente 15 a cada 3h) e **segue acelerado no momento** ate esse novo backlog zerar. Jogos sem nenhuma conquista na Steam ficam perpetuamente pendentes nessa fila (nunca geram linha em `game_achievements`), o que e esperado.
